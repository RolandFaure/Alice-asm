...
void assembly_custom2(std::string read_file, int min_abundance, bool contiguity, int size_longest_read, std::string tmp_folder, int num_threads, std::string final_gfa, std::vector<int> kmer_sizes_vector, bool single_genome, std::string path_to_bcalm, std::string path_convertToGFA, std::string path_graphunzip){
    
    time_t now2 = time(0);
    tm *ltm2 = localtime(&now2);

    int kmer_len = 31;
    vector<int> values_of_k = {31};
    ...
    int abundancemin = single_genome ? min_abundance : 2;
    ...
    cout << "       - Shaving the graph of small dead ends [" << 1+ ltm2->tm_mday << "/" << 1 + ltm2->tm_mon << "/" << 1900 + ltm2->tm_year << " " << ltm2->tm_hour << ":" << ltm2->tm_min << ":" << ltm2->tm_sec << "]" << endl;
    string shaved_gfa = tmp_folder+"bcalm"+std::to_string(kmer_len)+".unitigs.shaved.gfa";
    pop_and_shave_graph(unitig_file_gfa, min_abundance, 2*kmer_len+1, contiguity, kmer_len, shaved_gfa, 0, num_threads, single_genome);
    auto time_shave = std::chrono::high_resolution_clock::now();
    ...
    if (contiguity){
        ...
        vector<Segment> segments;
        vector<Segment> merged_segments;
        load_GFA(shaved_and_popped_gfa, segments, segments_IDs, true);
        string shaved_and_popped_merged = tmp_folder+"bcalm.unitigs.shaved.popped.merged.gfa";
        merge_adjacent_contigs(segments, merged_segments, shaved_and_popped_gfa, true, num_threads);
        output_graph(shaved_and_popped_merged, shaved_and_popped_gfa, merged_segments);
        merged_gfa = shaved_and_popped_merged;
    }
    auto time_pop = std::chrono::high_resolution_clock::now();
    ...
    string gaf_file = tmp_folder+"bcalm.unitigs.shaved.merged.unzipped.gaf";
    unordered_map<string,float> coverages;
    create_gaf_from_unitig_graph(merged_gfa, kmer_len, read_file, gaf_file, coverages);   
    auto time_gaf = std::chrono::high_resolution_clock::now(); 
    now2 = time(0);
    ltm2 = localtime(&now2);
    cout << "    - Untangling the graph with GraphUnzip [" << 1+ ltm2->tm_mday << "/" << 1 + ltm2->tm_mon << "/" << 1900 + ltm2->tm_year << " " << ltm2->tm_hour << ":" << ltm2->tm_min << ":" << ltm2->tm_sec << "]" << endl;
   
    string fake_fasta = tmp_folder + "fake_fasta.fa";

    // Process GAF file and output results
    vector<SuperRead> super_reads = process_gaf_file(gaf_file, kmer_len, 0);
    
    // Output the super reads
    string output_file = tmp_folder + "super_reads.txt";
    ofstream out(output_file);
    out << "# Final super reads from K2000\n";
    for (const auto& sr : super_reads) {
        for (size_t i = 0; i < sr.unitigs.size(); ++i) {
            if (i > 0) out << " ";
            int unitig = sr.unitigs[i];
            if (unitig < 0) {
                out << "<" << -unitig << ">";
            } else {
                out << "<" << unitig << ">";
    ...
    cout << "    - Processed " << super_reads.size() << " super reads, output to " << output_file << endl;
